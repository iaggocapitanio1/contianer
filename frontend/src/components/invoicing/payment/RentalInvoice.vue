<template>
  <div
    v-if="(!computedOrder || !customerOrderStore.publicCms) && state.loading"
  ...
  <div
    v-if="computedOrder && customerOrderStore.publicCms"
    class="flex-auto border rounded-border bg-0 dark:bg-950"
  >
    <PaymentHeader
      v-if="!mdAndSmaller"
      :orderDetails="computedOrder"
      :cms="customerOrderStore.publicCms"
      :isDelivery="isDelivery()"
      :displayMessage="displayMessage"
      :selectedRentPeriodId="$route.currentRoute.value.params.rentPeriodId"
    />
    <PaymentHeaderMobile
      v-if="mdAndSmaller"
      ...
      <div class="col-span-12 mb-4 md:col-span-8">
        <RentalCart
          ...
          :showAbbrevTitleWContainerNumber="
            userStore?.cms?.rent_options?.show_abbrev_title_w_container_number
          "
        />
      </div>
    </div>
    ...
    >
      <RentalInvoiceTable
        :verticalTable="smAndSmaller"
        :order="customerOrderStore.publicOrder"
        :rentPeriodId="$route.currentRoute.value.params.rentPeriodId"
      />
    </div>
    ...
    <div
      v-if="state.can_pay_on_delivery_order"
    ...
    <div
      class="flex flex-col items-center w-full"
      v-else-if="customerOrderStore.publicCms?.displays_delivery_period"
    >
...
<script setup>
  ...
  import RentalInvoiceTable from "./RentalInvoiceTable.vue"
  ...
  import RentalInvoiceCartSummary from "@/components/cart/RentalInvoiceCartSummary.vue"
  ...
  const computedOrder = computed(() => {
    return customerOrderStore.publicOrder || customerOrderStore.order
  })
  ...
  const fetchSchema = async () => {
    const result = await customerApi.getApplicationSchemasByName(
      can_pay_on_delivery.value ? "POD" : customerOrderStore.publicOrder.type,
  ...
  const displayMessage = computed(() => {
    ...
    if (pod_signed.value && customerOrderStore.publicOrder.type == "PURCHASE") {
  ...
  const summaryItems = computed(() => {
    ...
    //unpaid rent periods
    let sumRentalPeriods = 0
    customerOrderStore.publicOrder.rent_periods.forEach((el) => {
      console.log(el)
      if (
        el.calculated_rent_period_total_balance != 0 &&
        new Date(el.start_date) < new Date()
      ) {
        sumRentalPeriods += el.amount_owed
      }
    })
    ...
    customerOrderStore.publicOrder.rent_periods.forEach((el) => {
      if (
        ...
        el.rent_period_fees.forEach((fee) => {
          if (fee.type.name != "DROP_OFF" && fee.type.name != "PICK_UP") {
            sumFees += fee.fee_amount
          }
        })
      }
    })
    ...
    customerOrderStore.publicOrder.rent_periods.forEach((el) => {
      if (
        ...
        sumTax += el.calculated_rent_period_tax
      }
    })

    if (
      customerOrderStore.publicOrder.rent_periods[0]
        .calculated_rent_period_total_balance > 0
    ) {
      if (
        customerOrderStore.publicOrder.first_payment_strategy ==
        "FIRST_MONTH_PLUS_DELIVERY"
      ) {
        returnList.push({
          rowTitle: "Drop Off",
          rowContent: $fc(
        ...
        let drop_off =
          customerOrderStore.publicOrder.rent_periods[0]
            .calculated_down_payment_fees / 2
        returnList.push({
          rowTitle: "Drop Off",
          rowContent: $fc(drop_off)
        })
    ...
    returnList.push({
      rowTitle: "Subtotal Rentals",
      rowContent: $fc(sumRentalPeriods)
    })

    returnList.push({
      rowTitle: "Total Fees",
      rowContent: $fc(sumFees)
    })

    returnList.push({
      rowTitle: "Total Tax",
      rowContent: $fc(sumTax)
    })
  ...
  onMounted(async () => {
    ...
    console.log(customerOrderStore.publicOrder)
  })
  const applicationOverride = computed(() => {
    return (
      customerOrderStore.publicOrder?.applications_overridden.filter(
        (app) => app.name === "rent" && app.overridden
      ).length > 0
    )
  })
  ...
  const notPaidCancelledExpiredEstimateQuote = computed(() => {
    ...
    return (
      !isCancelled.value && !isExpired() && !isEstimate.value && !isQuote.value
    )
  })
