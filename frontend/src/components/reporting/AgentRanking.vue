  ...
  <div class="card">
    ...
    >
      ...
      <Column
        ...
        header="Max Units Sold Year"
      ></Column>
      <Column field="max_units_sold" header="Max Units Sold"></Column>

      <Column
        field="highest_sales_year_month"
        header="Highest Sales Year Month"
      ></Column>
      <Column field="highest_sales" header="Highest Sales"></Column>

      <Column field="current_sales" header="Date Range Sales"></Column>
      <Column field="current_units" header="Date Range Units"></Column>
    </DataTable>
  </div>
</template>
...
<script setup>
  ...
  const handleMultiSelectChange = async () => {
    ...
    await reportsApi.runByName("top_agents", dataObj)
    intervalManager.runFunctionInInterval(
      runFunctionInInterval("top_agents", dataObj, function (result) {
  ...
  const exportPdf = async (id, file_name) => {
    await exportHtmlToPdf(id, file_name, "Agent Ranking")
      .then((r) => {})
      .catch((e) => {
        console.log(e)
      })
  ...
  const exportData = async (type) => {
    type.title = type.title.replace("[INSERT_REPORT_TITLE]", "Agent Ranking")
