...
def get_credit_cards_post_processor_taxable(data):
    line_item_list = []
    ...
    for order_credit_card in data['order_credit_cards']:
        ...
        tax_percent = order._calculate_purchase_tax_percent()
        ...
        if (data['purchase_type'] == 'PURCHASE' or data['purchase_type'] == 'PURCHASE_ACCESSORY') and order.calculated_order_tax() > 0:
            for transaction in order_total_paid_dict[order.id]:
                ...
                result = {
                    ...
                    "subtotal_paid": transaction['amount'] - transaction['amount'] / (1 + tax_percent) * tax_percent,
                    "total_tax_paid": transaction['amount'] / (1 + tax_percent) * tax_percent,
