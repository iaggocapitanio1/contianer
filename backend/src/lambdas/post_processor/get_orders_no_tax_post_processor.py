...
def get_orders_no_tax_post_processor(data):
    line_item_list = []
    ...
    for transaction_type in data['transaction_types']:
        ...
        if data['purchase_type'] == 'PURCHASE' or data['purchase_type'] == 'PURCHASE_ACCESSORY':
            ...
            if transaction_type.rent_period:
                order = transaction_type.rent_period.order
        ...
        if order.id not in order_total_paid_dict:
            order_total_paid_dict[order.id] = [
                {"amount": transaction_type.amount, "created_at": transaction_type.created_at}
            ]
        else:
    ...
    all_transactions_and_cc = data['transaction_types'] + data['order_credit_cards']
    for transaction_type in all_transactions_and_cc:
        order = None
        if data['purchase_type'] == 'PURCHASE' or data['purchase_type'] == 'PURCHASE_ACCESSORY':
            order = transaction_type.order
        else:
            if isinstance(transaction_type, OrderCreditCard):
                order = transaction_type.order
            elif transaction_type.rent_period:
                order = transaction_type.rent_period.order
        ...
        if (
            (data['purchase_type'] == 'PURCHASE' or data['purchase_type'] == 'PURCHASE_ACCESSORY')
            and order.calculated_order_tax() == 0
            or data['purchase_type'] == 'RENT'
            and hasattr(order, "rent_periods")
            and len(order.rent_periods.related_objects) > 0
            and order.rent_periods[0].calculated_rent_period_tax() == 0
        ):
            for transaction in order_total_paid_dict[order.id]:
                ...
                result = {
                    ...
                    else order.customer.company_name
                    if order.customer
                    else None,
