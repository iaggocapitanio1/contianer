...
def get_container_sales_by_condition_percentages(filters: FilterObject, psycopg2_connection=None):
            ...
            sql.SQL(
                """
                    SELECT
                        ...
                        (SUM(CASE WHEN li.condition = 'Used' THEN 1 ELSE 0 END)::float/COUNT(*)::float * 100.0)::int AS total_containers_used,
                        (SUM(CASE WHEN li.condition = 'One-Trip' THEN 1 ELSE 0 END)::float/COUNT(*)::float * 100.0)::int AS total_containers_one_trip
                    FROM
                        "order" o
        ...
        query = (
            sql.SQL(
                """
                    ...
                    WHERE
                        o.status <> 'Invoiced' AND o.status <> 'Expired' AND o.paid_at BETWEEN {begin_date}
                            AND {end_date} AND cpn.container_size || ' ' || cpn.condition ||
                        CASE
                            WHEN EXISTS (
                                SELECT *
